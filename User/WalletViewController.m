    ...
    if ([appDelegate internetConnected])
    {
        ...
        [afn getDataFromPath:MD_GETPROFILE withParamData:params withBlock:^(id response, NSDictionary *error, NSString *errorcode) {
            ...
            if (response)
            {
                ...
                [user1 setValue:_amountLbl.text forKey:@"Wallet"];

                
                if ([_amountLbl.text isEqualToString:@"0"])
                {
    ...
    if ([appDelegate internetConnected])
    {
        ...
        [afn getDataFromPath:MD_LIST_CARD withParamData:nil withBlock:^(id response, NSDictionary *error, NSString *errorcode) {
            ...
            if (response)
            {
                ...
                {
                    ///Add cards
                    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LocalizedString(@"ALERT")  message:LocalizedString(@"Please add a card to proceed") preferredStyle:UIAlertControllerStyleAlert];
...
-(IBAction)addBtn:(id)sender
{
    ...
    {
        ///Add cards
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:LocalizedString(@"ALERT")  message:LocalizedString(@"ADD_CARD") preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction* ok = [UIAlertAction actionWithTitle:LocalizedString(@"OK") style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            CreditCardViewController *wallet = [self.storyboard instantiateViewControllerWithIdentifier:@"CreditCardViewController"];
            [self presentViewController:wallet animated:YES completion:nil];
        }];
...
-(IBAction)addMoneyBtn:(id)sender
{
    ...
    {
        [CommenMethods alertviewController_title:LocalizedString(@"Alert") MessageAlert:LocalizedString(@"AMOUNT_WALLET") viewController:self okPop:NO];
    }
    else
    {
        if ([appDelegate internetConnected])
