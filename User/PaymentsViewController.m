...
-(void)LocalizationUpdate{
    _navicationTitleLbl.text = LocalizedString(@"Payments");
}
-(void)getAllCards
{
    if ([appDelegate internetConnected])
    {
        ...
        [afn getDataFromPath:MD_LIST_CARD withParamData:nil withBlock:^(id response, NSDictionary *error, NSString *errorcode) {
            ...
            if (response)
            {
                self->arrPaymentCardList=[[NSMutableArray alloc] init];
                
                if ([self->_fromWhereStr isEqualToString:@"WALLET"])
                {
                ...
                else
                {
                    ...
                    [self->arrPaymentCardList addObject:dictCash];
                }
                
                
                [self->arrPaymentCardList addObjectsFromArray:response];
                if(self->arrPaymentCardList.count)
                  {
                      NSLog(@"%@",self->arrPaymentCardList);
                      [self->_payTableView reloadData];
                  }
                else {
